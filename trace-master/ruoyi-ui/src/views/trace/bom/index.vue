<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="生产计划单编号" prop="productionPlanningCode" label-width="85">
        <el-input
          v-model="queryParams.productionPlanningCode"
          placeholder="请输入生产计划单编号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="生产批号" prop="productionBatchCode">
        <el-input
          v-model="queryParams.productionBatchCode"
          placeholder="请输入生产批号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="灭菌批号" prop="productionSterilizationCode">
        <el-input
          v-model="queryParams.productionSterilizationCode"
          placeholder="请输入灭菌批号"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      <el-form-item label="入库库房编号" prop="productionWarehouseCode" label-width="85">
        <el-select v-model="queryParams.productionWarehouseCode" placeholder="库房编号" clearable>
          <el-option
            v-for="item in warehousingList"
            :key="item.productionWarehousingCode"
            :label="item.productionWarehouseCode"
            :value="item.productionWarehouseCode"
            />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="库管员" prop="productionWarehouseKeeper">
        <el-input
          v-model="queryParams.productionWarehouseKeeper"
          placeholder="请输入库管员"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="制单人" prop="productListCreator">
        <el-input
          v-model="queryParams.productListCreator"
          placeholder="请输入制单人"
          clearable
          @keyup.enter.native="handleQuery"
        />
...
export default {
  ...
  methods: {
    ...
    getDetailList(){
      listDetailAll().then(resposen => {
        this.detailList = resposen.rows;
      })
    },
    // 取消按钮
    cancel() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除生产入库编号为"' + ids + '"的数据项？').then(function() {
